...
const reportSource = [
  ...
  "",
  `Product Intelligence Milestone 1 has ${migrationFiles.length} versioned migrations and ${databaseAssertionCount} declared pgTAP assertions across ${databaseTestFiles.length} test files. The prior candidate's local runtime proof is recorded in the Milestone 1 operations runbook, but it does not automatically validate later migration or test edits. Fresh local replay and hosted non-production staging parity are still required, so Console Milestone 2 remains blocked.`,
  "",
  "## SKU Milestones",
  "",
