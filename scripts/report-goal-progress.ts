...
const reportSource = [
  ...
  "",
  `Product Intelligence Milestone 1 has ${migrationFiles.length} versioned migrations and ${databaseAssertionCount} declared pgTAP assertions across ${databaseTestFiles.length} test files. The Milestone 1 operations runbook records candidate-specific reset, pgTAP, generated-type and exact-row import evidence, plus the owner-authorized staging destination. Passing isolated-runtime results cover only the tested candidate and do not establish hosted parity. Successful replay in that exact non-production project remains required before Console Milestone 2.`,
  "",
  "## SKU Milestones",
  "",
